    ...
    if (termSheet) {
      ...
      if (termData.length > 1) {
        ...
        termRows.forEach(row => {
           let statusStr = String(row[2] || '');
           let stepStr = String(row[3] || '');
           if (!row[0]) return; // empty row skip
           flows.push({
             id: String(row[0] || ''),
             employee: String(row[5] || ''),
             status: statusStr,
             step: stepStr,
             requesterName: String(row[4] || ''), // reqEmail
             requesterEmail: String(row[4] || ''),
             initiator: 'Termination',
             dateRequested: row[1] instanceof Date ? row[1].toLocaleDateString() : String(row[1] || ''),
             lastUpdated: row[1] instanceof Date ? row[1].toLocaleString() : String(row[1] || ''),
...
/**
 * Get Details for Termination Workflow Modal
 */
  ...
  try {
     ...
     reqHeaders.forEach((h, i) => {
       if (h) {
         ...
         if (h === 'Employee Name') r['First Name'] = val;
         if (h === 'Site Name') r['Site'] = val;
         if (h === 'Termination Date') r['Start Date'] = val;
         if (h === 'Manager Name') r['Job Title'] = 'Manager: ' + val;
     ...
     if (aiSheet) {
        ...
        if (aiData.length > 1) {
          ...
          const assignIdx = aiHeaders.indexOf('Assignee');
          const dateIdx = aiHeaders.indexOf('Created Date'); // or Timestamp
          ...
          for (let i = 1; i < aiData.length; i++) {
             if (aiData[i][wfIdx] === workflowId) {
                ...
                if (!categories[cat]) categories[cat] = { total: 0, complete: 0, by: aiData[i][assignIdx], time: aiData[i][dateIdx] };
                categories[cat].total++;
                if (aiData[i][statusIdx] === 'Complete') categories[cat].complete++;
             }
          }
          ...
          for (let cat in categories) {
             ...
             checklist.push({
                 ...
                 status: status,
                 by: c.by || '-',
                 time: c.time instanceof Date ? c.time.toLocaleString() : String(c.time || '')
             });
          }
        }
     }
