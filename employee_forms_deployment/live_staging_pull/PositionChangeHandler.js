  ...
  try {
    ...
    if (decision === 'Approved') {
      ...
      if (nextSteps && nextSteps.includes('IT')) {
        ...
        systems.forEach(sys => {
          const tid = ActionItemService.createActionItem(workflowId, 'IT', `Setup ${sys}`, `Provision access for ${changeData.empName}`, CONFIG.EMAILS.IT);
        ...
        assets.forEach(asset => {
          const tid = ActionItemService.createActionItem(workflowId, 'IT', `Provision ${asset}`, `Setup and deliver ${asset} for ${changeData.empName}`, CONFIG.EMAILS.IT);
        ...
        if (itTasks.length > 0) {
          ...
          sendFormEmail({
            to: CONFIG.EMAILS.IT,
            subject: `IT Action Required: Status Change Tasks - ${changeData.empName}`,
            body: `HR has approved a status change for ${changeData.empName}. Below are the individual action items that require closure:<br><br>${taskLinks}`,
            contextData: changeData
          });
        }
      }
