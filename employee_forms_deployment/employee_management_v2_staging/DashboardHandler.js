  ...
  try {
     ...
     reqHeaders.forEach((h, i) => {
       if (h) {
         ...
         if (h === 'Site') r['Site'] = val;
         if (h === 'Term Date') r['Start Date'] = val;
         if (h === 'Manager Email') r['Job Title'] = 'Manager Email: ' + val;
         if (h === 'Requester Email') r['Requester Email'] = val;
        ...
        if (aiData.length > 1) {
          ...
          for (let i = 1; i < aiData.length; i++) {
             if (aiData[i][wfIdx] === workflowId) {
                ...
                if (aiData[i][statusIdx] === 'Closed' || aiData[i][statusIdx] === 'Complete') categories[cat].complete++;
             }
          }
