...
function makeRuntime() {
  ...
  const Utilities = {
    ...
    // Digest support so FormContracts.hash_ produces the same 'sha256:…' as live Apps Script (review pass 2 M3).
    // GAS returns signed bytes (-128..127); FormContracts normalises with (b + 256) % 256.
    DigestAlgorithm: { MD5: 'md5', SHA_1: 'sha1', SHA_256: 'sha256', SHA_384: 'sha384', SHA_512: 'sha512' },
    Charset: { UTF_8: 'utf8', US_ASCII: 'ascii' },
    computeDigest(alg, value, charset) {
      const crypto = require('crypto');
      const buf = crypto.createHash(alg || 'sha256').update(typeof value === 'string' ? Buffer.from(value, charset === 'ascii' ? 'ascii' : 'utf8') : Buffer.from(value)).digest();
      return Array.from(buf, b => (b > 127 ? b - 256 : b));
    },
    computeHmacSha256Signature(value, key) {
      const crypto = require('crypto');
      const buf = crypto.createHmac('sha256', typeof key === 'string' ? Buffer.from(key, 'utf8') : Buffer.from(key)).update(typeof value === 'string' ? Buffer.from(value, 'utf8') : Buffer.from(value)).digest();
      return Array.from(buf, b => (b > 127 ? b - 256 : b));
    },
  ...
  // NOTE: the store is created ONCE per runtime, not per getScriptProperties() call. It used to be
  // re-created on every call, so setProperty() was invisible to the next getProperty() and any code
  // reading a Script Property could not be tested at all (see the SAFETY_TRAINING_AT_SUBMIT skip).
  const _props = {};
  const _propStore = {
    getProperty(key)          { return _props[key] || null; },
    setProperty(key, val)     { _props[key] = String(val); return this; },
    getProperties()           { return { ..._props }; },
    setProperties(obj)        { Object.assign(_props, obj); return this; },
    deleteProperty(key)       { delete _props[key]; return this; },
    deleteAllProperties()     { Object.keys(_props).forEach(k => delete _props[k]); return this; }
  };
  const PropertiesService = {
    getScriptProperties() { return _propStore; },
    getUserProperties()   { return _propStore; }
  };
