...
var ActionItemService = (function() {
   ...
   */
  function closeActionItem(taskId, notes, closedBy) {
    try {
  ...
  function notifyTaskClosure(taskId, notes, closedBy) {
    try {
      ...
      const body = `The following action item has been marked as <b>Closed</b>.
      ...
      // Notify the requester/manager if it's an Asset task or initiator
      const recipients = [];
      if (workflow && workflow['Initiator Email']) recipients.push(workflow['Initiator Email']);
  ...
  function notifyWorkflowClosure(workflowId) {
    try {
      ...
      // Notify Initiator
      if (workflow['Initiator Email']) {
        sendFormEmail({
          to: workflow['Initiator Email'],
          subject: subject,
          body: body,
          contextData: context
        });
 ...
 */
function closeActionItemWithNotes(taskId, notes) {
  return ActionItemService.closeActionItem(taskId, notes, Session.getActiveUser().getEmail());
}
